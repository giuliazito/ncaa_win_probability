...
## Overview
An end-to-end machine learning pipeline and interactive web application designed to forecast NCAA basketball game outcomes from a public dataset. This project uses chronological, play-by-play event logs and applies an advanced Extreme Gradient Boosting (XGBoost) classification model to evaluate game states in real-time.
...
A Logistic Regression baseline model was trained to capture linear trends, and achieved a strong initial AUC. Non-linear momentum shifts, such as dramatic late-game scoring run, were modeled using an XGBoost classifier. Hyperparameters (max_depth=5, learning_rate=0.1) were explicitly tuned to prevent overfitting, mathematically validated by comparing Training AUC (0.8883) against Testing AUC (0.9038). 

### 4. Deployment & Interactive UI
The optimized model was serialized and deployed into a live, interactive web application using Streamlit. The dashboard allows users to input hypothetical game scenarios via custom UI elements, instantly returning dynamic win-probability metrics and visual progress indicators. At this time, teams do not affect the probability score, as they only serve as user experience. 
